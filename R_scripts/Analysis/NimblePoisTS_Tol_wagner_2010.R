poisTSCode <- nimbleCode({
   if(K > 1){
      A[1:K,1:K] <- inverse(diag(x=rep(100,K)))
      multimu[1:K] <- rep(0,K)
...
T_Luterbacher2016 <- EuroClimCon$T_luterbacher2016[index]
T_Buntgen2011 <- EuroClimCon$T_Buntgen2011_JJA[index]
T_Glaser2009 <- EuroClimCon$T_Glaser2009[index]
...
#save(samples,file=paste("../Results/MCMC_Chains/",timespan,"/model_",modelnum,".RData",sep=""))
...
#write.csv(t(mcmc_geweke$z),file=paste("../Results/MCMC_Chains/#",timespan,"/#geweke_model_",modelnum,".csv",sep=""))

#plots
samples_trim <- samples[1000:niter,grep("B|rho|sigma|lambda0",colnames(samples))]
source("../R_scripts/Plotting/plot_mcmc.R")
#dev.copy(png,file=paste("../Results/MCMC_Chains/",timespan,"/#model_",modelnum,".png",sep=""),height=1000,width=1000,units="px",res=150)
#dev.off()
