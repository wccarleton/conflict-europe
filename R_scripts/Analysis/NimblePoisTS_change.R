poisTSCode <- nimbleCode({
   if(K > 1){
      ...
      mu[1] <- inprod(X[1,1:K], ((B_1[1:K] * (1-ind[1])) + (B_2[1:K] * ind[1])))
   }else{
      B_1 ~ dnorm(mean=0,sd=2)
      B_2 ~ dnorm(mean=0,sd=2)
      mu[1] <- X[1] * ((B_1 * (1-ind[1])) + (B_2 * ind[1]))
   }
   sigma ~ dexp(0.1)
   ...
   for (j in 2:T){
      if(K > 1){
         mu[j] <- inprod(X[j,1:K], ((B_1[1:K] * (1-ind[j])) + (B_2[1:K] * ind[j])))
      }else{
         mu[j] <-  X[j] * ((B_1 * (1-ind[j])) + (B_2 * ind[j]))
      }
